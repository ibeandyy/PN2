...
export default class BuyRaven extends Command {
    name = "buyraven";
    aliases = [];
    ...
    async exec(msg: Message, args: string[]) {
      ...
      const item = "Raven";
      const role = "984202147900063784";
      const parrot = "980959662906150963";
      const price = 15;
      var charge = 0;
      charge -= price;
      ...
      if (author != null) {
        ...
        if (userID !== undefined) {
          ...
          if (moneyUser.cash > price && author.roles.cache.has(role) == false) {
           ...
           if (author.roles.cache.has(parrot)) await author.roles.remove(parrot).catch((e) => console.log(e));
