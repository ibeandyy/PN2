...
export default class BuySwashbuckle extends Command {
    name = "buyswashbuckle";
    aliases = [];
    ...
    async exec(msg: Message, args: string[]) {
      ...
      const item = "Swashbuckle";
      const role = "984202211221454928";
      const rally = "984203254550392862";
      const price = 25;
      var charge = 0;
      charge -= price;
      ...
      if (author != null) {
        ...
        if (userID !== undefined) {
          ...
          if (moneyUser.cash > price && author.roles.cache.has(role) == false) {
           ...
           if (author.roles.cache.has(rally)) await author.roles.remove(rally).catch((e) => console.log(e));
