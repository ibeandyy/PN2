...
export default class BuyBreeches extends Command {
    name = "buybreeches";
    aliases = [];
    ...
    async exec(msg: Message, args: string[]) {
      ...
      const item = "Leather Breeches";
      const role = "984201402559639572";
      const price = 10;
      var charge = 0;
      charge -= price;
      ...
      if (author != null) {
        ...
        if (userID !== undefined) {
          ...
          if (moneyUser.cash > price && author.roles.cache.has(role) == false) {
           ...
           .setColor(BROWN)
           .addField("Thanks for yer patronage!", `You are now the proud owner of a set of ${item}, that'll be 1 silver!`, false)
           ...
           .setTitle("Typho Redbeard's Marketplace")
           .setColor(BROWN)
           .addField("Wait just a minute!", `First off, you cannot afford a set of${item}\nFurthermore, you've already got one of the finest sets of ${item} we offer!`, false)
           msg.channel.send({ embeds: [embed] });  
           ...
           .setTitle("Typho Redbeard's Marketplace")
           .setColor(BROWN)
           .addField("Wait just a minute!", `You cannot afford a set of ${item}!\nMaybe do some gamblin' or stealin'...but no from me!`, false)
           msg.channel.send({ embeds: [embed] });  
           ...
           .setTitle("Typho Redbeard's Marketplace")
           .setColor(BROWN)
           .addField("Wait just a minute!", `You've already got one of the finest sets of ${item} we offer!\nLeave some for the others!`, false)
           msg.channel.send({ embeds: [embed] });  
